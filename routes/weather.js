    ...
    db.one("SELECT * FROM WEATHER WHERE zip = $1", zipcode)
        .then(row => {
            ...
            if (parsed[0] != d.getFullYear() || parsed[1] != d.getMonth() || parsed[2] != d.getDate() || parsed[3] != (d.getHours())) {
                ...
                .then(row => {
                    let body = [row['dailyweather'], row['hourlyweather'], JSON.stringify("zip\":\"" + row['zip'])];
    ...
    request(url, function (error, response, body) {
        if (error) {
            ...
            .then(row => {
                ...
                //body = dailyweather;
                body = [JSON.parse(dailyweather), JSON.parse(hourlyweather), JSON.stringify("zip\":\"" + row['zip'])];
